...
直观来想，一个motif的得分，应该是受motif边的得分影响，边的得分越高，motif得分也应该越高，因此，应当可以重新使用链路预测的得分函数。

motif的得分函数s是与Vm 和 （Em ∪ -Em，d）有关的。如果我们重用一阶的链路预测方法，比如jaccard方法，那么s就与Vm和直接相邻的邻居有关。如果是像Adamic-Adar的高阶链路预测方法，那么较大一部分邻居Vm都会被考虑在计算s内。这里，本篇进行的评估表明，考虑1到2跳就能实现高预测精度。

#### 无链接的启发式方法

如果使用Jaccard分数$s(e)$，则motif预测分数为
$$
s(M)^J = \prod_{e_{u,v} \in E_{M,N}} \frac{|N_u∩N_v|}{|N_u∪N_v|}
$$
为了包含交易破坏者的边，把上式推广，得到
$$
s^*(M) = \prod_{e \in E_M}s(e)\cdot \prod_{e \in \mathop{E_{M,D}} \limits^-}(1 - s(e))
$$
也就很容易理解，这里e这条边出现机会越大，$s(e)$评分越高，使用$1-s(e)$来控制评分，使得存在Deal-Breaker边时，具有期望的递减状态。
...
然后内标签转换成一个独热（one-hot）矩阵$H∈N^{k\times k}$。其中Hij代表的是，第i个节点的内标签是j。
